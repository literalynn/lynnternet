...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const CFG = Object.freeze({ MAX_HISTORY: 30, POLL_OK_MS: 2000, POLL_DOWN_MS: 15000, PING_PANELS_MS: 20000, FETCH_TIMEOUT_MS: 10000, INITIAL_BACKOFF_MS: 5000, MAX_BACKOFF_MS: 60000 });
  ...
  let writeQueue = [], rafScheduled = false;
  function scheduleWrite(fn) { writeQueue.push(fn); if (!rafScheduled) { rafScheduled = true; requestAnimationFrame(() => { const q = writeQueue; writeQueue = []; rafScheduled = false; for (const f of q) { try { f(); } catch {} } }); } }
  ...
  function createBiaxialFillPattern(chart, area, color, { topAlpha = 0.25, fadeRatio = 0.4 } = {}) {
    const off = document.createElement('canvas'); off.width = Math.max(1, area.right - area.left); off.height = Math.max(1, area.bottom - area.top);
    const g = off.getContext('2d'); const vGrad = g.createLinearGradient(0, 0, 0, off.height);
    vGrad.addColorStop(0, makeAlpha(color, topAlpha)); vGrad.addColorStop(1, makeAlpha(color, 0)); g.fillStyle = vGrad; g.fillRect(0, 0, off.width, off.height);
    g.globalCompositeOperation = 'destination-in'; const fr = Math.min(Math.max(fadeRatio, 0), 1); const hMask = g.createLinearGradient(0, 0, off.width, 0);
    hMask.addColorStop(0, 'rgba(0,0,0,0)'); hMask.addColorStop(fr, 'rgba(0,0,0,1)'); hMask.addColorStop(1, 'rgba(0,0,0,1)'); g.fillStyle = hMask; g.fillRect(0, 0, off.width, off.height);
    return chart.ctx.createPattern(off, 'no-repeat');
  }
  const glowLinePlugin = { id: 'glow', beforeDatasetDraw(chart, args, options) { const { ctx } = chart; ctx.save(); const ds = chart.data.datasets[args.index]; const base = ds._baseBorderColor || ds.borderColor || '#fff'; ds._baseBorderColor = base; ctx.shadowBlur = (prefersReduced ? 0 : (options?.shadowBlur ?? 20)) * DPR; ctx.shadowColor = typeof base === 'string' ? base : '#fff'; ctx.shadowOffsetX = 0; ctx.shadowOffsetY = 0; }, afterDatasetDraw(chart) { chart.ctx.restore(); } };
  const fadeLeftPlugin = { id: 'fadeLeft', beforeDatasetsDraw(chart, _args, pluginOptions) { const { ctx, chartArea } = chart; if (!chartArea) return; const fadeRatio = pluginOptions?.fadeRatio ?? 0.4; for (const ds of chart.data.datasets) { if (!ds._baseBorderColor) ds._baseBorderColor = ds.borderColor; const base = ds._baseBorderColor; if (typeof base !== 'string') continue; const grad = ctx.createLinearGradient(chartArea.left, 0, chartArea.right, 0); grad.addColorStop(0, base.replace('rgb', 'rgba').replace(')', ',0)')); grad.addColorStop(fadeRatio, base); ds.borderColor = grad; ds.pointBackgroundColor = base; } } };
  if (window.Chart) Chart.register(glowLinePlugin, fadeLeftPlugin);
  function createChartConfig(mainColor) {
    return { type: 'line', data: { labels: [], datasets: [{ data: [], borderColor: mainColor, borderWidth: 2, fill: true, tension: 0.4, pointRadius: 0, pointHoverRadius: 5, pointBackgroundColor: mainColor, pointHoverBorderColor: '#fff', pointHoverBorderWidth: 2 }] }, options: { normalized: true, animation: prefersReduced ? false : { duration: 0 }, responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, max: 100, ticks: { color: '#a0a0a0' }, grid: { color: 'rgba(255,255,255,0.08)', drawBorder: false } }, x: { ticks: { display: false }, grid: { display: false } } }, plugins: { legend: { display: false }, tooltip: { enabled: true, mode: 'index', intersect: false, backgroundColor: 'rgba(12,12,12,0.5)', titleColor: '#fff', bodyColor: '#fff', padding: 10, cornerRadius: 16, borderColor: 'rgba(255,255,255,0.1)', borderWidth: 1 }, glow: { shadowBlur: prefersReduced ? 0 : 24 }, fadeLeft: { fadeRatio: 0.4 } } } };
  }
  ...
  if (isDashboard) { SERVERS.forEach(s => serversContainer.insertAdjacentHTML('beforeend', serverSectionHTML(s))); if (statusSummaryContainer) SERVERS.forEach(s => statusSummaryContainer.insertAdjacentHTML('beforeend', statusSummaryHTML(s))); }
  ...
  function stateFor(s) {
    ...
    const st = states[s.id] = {
      ...
      charts: {}, retryCount: 0, isConnected: false, disksInitialized: false, diskElements: {},
      ...
      persistTick: 0, backoffMs: 0, nextAllowedAt: 0, controller: null
    };
    return st;
  }
  if (isDashboard) {
    SERVERS.forEach(s => { const snap = loadSession(`lastStats_${s.id}`); if (snap) paintStats(s, stateFor(s), snap, false); initCharts(s); });
    tickAll();
  ...
  function initCharts(s) {
    ...
    cpuCfg.data.datasets[0].backgroundColor = ({ chart }) => {
      const area = chart.chartArea; if (!area) return makeAlpha(CPU_COLOR, 0.25);
      if (!cpuPattern || cpuW !== chart.canvas.width || cpuH !== chart.canvas.height) { cpuW = chart.canvas.width; cpuH = chart.canvas.height; cpuPattern = createBiaxialFillPattern(chart, area, CPU_COLOR, { topAlpha: 0.25, fadeRatio: 0.4 }); }
      return cpuPattern;
    };
    ...
    ramCfg.data.datasets[0].backgroundColor = ({ chart }) => {
      const area = chart.chartArea; if (!area) return makeAlpha(RAM_COLOR, 0.25);
      if (!ramPattern || ramW !== chart.canvas.width || ramH !== chart.canvas.height) { ramW = chart.canvas.width; ramH = chart.canvas.height; ramPattern = createBiaxialFillPattern(chart, area, RAM_COLOR, { topAlpha: 0.25, fadeRatio: 0.4 }); }
      return ramPattern;
    };
    st.charts.ram = new Chart(ramCanvas, ramCfg);
    let dlPattern, dlW = 0, dlH = 0, ulPattern, ulW = 0, ulH = 0;
    ...
    netCfg.data.datasets[0].backgroundColor = ({ chart }) => {
      const area = chart.chartArea; if (!area) return makeAlpha(DOWNLOAD_COLOR, 0.25);
      if (!dlPattern || dlW !== chart.canvas.width || dlH !== chart.canvas.height) { dlW = chart.canvas.width; dlH = chart.canvas.height; dlPattern = createBiaxialFillPattern(chart, area, DOWNLOAD_COLOR, { topAlpha: 0.25, fadeRatio: 0.4 }); }
      return dlPattern;
    };
    netCfg.data.datasets.push({ ...netCfg.data.datasets[0], label: 'Upload', borderColor: UPLOAD_COLOR, backgroundColor: ({ chart }) => {
      const area = chart.chartArea; if (!area) return makeAlpha(UPLOAD_COLOR, 0.25);
      if (!ulPattern || ulW !== chart.canvas.width || ulH !== chart.canvas.height) { ulW = chart.canvas.width; ulH = chart.canvas.height; ulPattern = createBiaxialFillPattern(chart, area, UPLOAD_COLOR, { topAlpha: 0.25, fadeRatio: 0.4 }); }
      return ulPattern;
    }, pointBackgroundColor: UPLOAD_COLOR });
    netCfg.options.scales.y.max = undefined; netCfg.options.scales.y.suggestedMax = 10; netCfg.options.plugins.tooltip.displayColors = true;
  ...
  function seedChart(chart, datasets) { if (!chart) return; const n = datasets[0]?.length || 0; chart.data.labels = Array(n).fill(''); datasets.forEach((ds, i) => { chart.data.datasets[i].data = ds; }); chart.update('none'); }
  function appendPoint(chart, dsIndex, value, max) { if (!chart) return; const ds = chart.data.datasets[dsIndex]; ds.data.push(value); if (ds.data.length > max) ds.data.shift(); if (chart.data.labels.length < ds.data.length) chart.data.labels.push(''); if (chart.data.labels.length > ds.data.length) chart.data.labels.shift(); chart.update('none'); }
  function fetchJSON(url, { timeout = CFG.FETCH_TIMEOUT_MS, signal } = {}) {
    const existing = inflightByUrl.get(url); if (existing) return existing;
    const ctrl = new AbortController(); const outerSignal = signal; const onAbort = () => ctrl.abort(); if (outerSignal) outerSignal.addEventListener('abort', onAbort, { once: true });
    const timeoutId = setTimeout(() => ctrl.abort(), timeout);
    const p = fetch(url, { method: 'GET', signal: ctrl.signal }).then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); }).finally(() => { clearTimeout(timeoutId); inflightByUrl.delete(url); if (outerSignal) outerSignal.removeEventListener('abort', onAbort); });
    inflightByUrl.set(url, p); return p;
  }
  function updateAPIStatus(s, status, message) {
    ...
    scheduleWrite(() => {
      ...
      if (mobileTile) { const icon = mobileTile.querySelector('.tile-icon'); if (mobileTxt) setText(mobileTxt, status === 'error' ? 'Hors ligne' : status === 'connecting' ? 'Connexion…' : 'Connecté'); if (icon) icon.textContent = status === 'connected' ? '✅' : status === 'error' ? '⚠️' : '⏳'; mobileTile.setAttribute('aria-hidden', status === 'connected' ? 'true' : 'false'); }
    });
  }
  function tickAll() {
    const now = Date.now();
    SERVERS.forEach(s => {
      const st = stateFor(s); if (st.nextAllowedAt && now < st.nextAllowedAt) return;
      ...
      fetchJSON(s.api, { timeout: CFG.FETCH_TIMEOUT_MS, signal: st.controller.signal }).then(stats => {
        st.retryCount = 0; st.isConnected = true; st.backoffMs = 0; st.nextAllowedAt = 0; updateAPIStatus(s, 'connected', 'Connecté'); persistSession(`lastStats_${s.id}`, stats); paintStats(s, st, stats, true);
      }).catch(() => {
        st.retryCount++; st.isConnected = false; updateAPIStatus(s, 'error', `Erreur – Retry ${st.retryCount}`); st.backoffMs = Math.min(CFG.MAX_BACKOFF_MS, st.backoffMs ? st.backoffMs * 2 : CFG.INITIAL_BACKOFF_MS); st.nextAllowedAt = Date.now() + st.backoffMs;
      });
    });
  }
  function paintStats(s, st, stats, seedChartsOnFirst) {
    ...
    if (st.charts.cpu && st.charts.ram && st.charts.net) { appendPoint(st.charts.cpu, 0, cpu, CFG.MAX_HISTORY); appendPoint(st.charts.ram, 0, ram, CFG.MAX_HISTORY); appendPoint(st.charts.net, 0, down, CFG.MAX_HISTORY); appendPoint(st.charts.net, 1, up, CFG.MAX_HISTORY); }
    else if (seedChartsOnFirst) setTimeout(() => paintStats(s, st, stats, false), 0);
    st.persistTick = (st.persistTick + 1) % 3; if (st.persistTick === 0) saveHistory(s, st);
    const { cpuText, ramText, netDownEl, netUpEl, storageText, loading } = st.elements;
    const cpuTxt = `${cpu.toFixed(1)}%`, ramTxt = `${ram.toFixed(1)}%`, downTxt = `↓ ${down.toFixed(1)}`, upTxt = `↑ ${up.toFixed(1)}`, storTxt = `${stats.storage_total_used} To`;
    scheduleWrite(() => {
  ...
  function updateDisksUI(s, st, disks) {
    ...
    if (needRebuild) {
      scheduleWrite(() => {
        ...
        for (const d of healthy) {
          ...
          const root = storageList.querySelector(`#${sid}`); if (!root) continue;
          st.diskElements[d.name] = { root, used: root.querySelector('.disk-value .used'), total: root.querySelector('.disk-value .total'), bar: root.querySelector('.progress-bar'), glow: root.querySelector('.progress-bar-shadow'), lastPercent: null, lastUsed: null, lastTotal: null };
        }
    ...
    scheduleWrite(() => {
      for (const d of healthy) {
        ...
        if (el.lastPercent !== pctStr) { el.bar.style.width = pctStr; el.glow.style.width = pctStr; el.lastPercent = pctStr; }
  ...
  if (!isDashboard && panelStatusCards && panelStatusCards.length) {
    ...
    map.forEach((cards, serverId) => {
      ...
      if (!s || !s.api) { cards.forEach(({ card, indicator, tooltip }) => setPanelCardStatus(card, indicator, tooltip, 'error', 'Hors ligne')); return; }
      ...
      ping(); setInterval(ping, CFG.PING_PANELS_MS);
    });
  }
